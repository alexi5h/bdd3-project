    ...
    public function prepare() {
        ...
        foreach ($tables as $table) {
            ...
            $this->files[] = new CCodeFile(
                    Yii::getPathOfAlias($this->modelPath . '.' . $className) . '.php', $this->render($templatePath . DIRECTORY_SEPARATOR . 'model.php', $params)
            );
            // Generate the base model.
            $this->files[] = new CCodeFile(
                    Yii::getPathOfAlias($this->baseModelPath . '.' . $this->baseModelClass) . '.php', $this->render(
                            $templatePath . DIRECTORY_SEPARATOR . '_base' . DIRECTORY_SEPARATOR . 'basemodel.php', $params
                    )
            );
        }
    }
    ...
    public function generateRules($table) {
        ...
        foreach ($table->columns as $name=>$column) {
            ...
            if (strncmp($column->dbType, 'enum', 4) === 0 && preg_match('/\((.*)\)/', $column->dbType, $matches)) {
                ...
                $enums[] = "array('{$name}', 'in', 'range' => array({$values})), // enum";
            }
        }
        if ($null !== array()) {
            $rules[] = "array('" . implode(
        ...
        return array_merge($this->parentGenerateRules($table), $enums, $rules );
    }
